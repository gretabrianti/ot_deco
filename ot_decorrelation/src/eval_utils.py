...
class EvalauteFramework:
    ...
    def ideal_calculation(self, background_rej = [0.1, 0.3, 0.5, 0.8], ax=None,
                          sample_times=50):
    ...
    def bkg_rej_calculation(self,column,background_rej = [0.1, 0.3, 0.5, 0.8],
        ...
        self.background_rej=background_rej
        print(len(bkg_mask))
        print(len(proba_flat))
        print(len(proba_flat[bkg_mask]))
        probabilities_bins = np.quantile(proba_flat[bkg_mask], background_rej)
        ...
        counts_truth, ax_1 = plot.plot_hist(self.conds[bkg_mask],
        ...
        for nr, prob_bin in enumerate(probabilities_bins):
            # sig_eff
            mask_proba = (proba_flat>=prob_bin) & (bkg_mask)
